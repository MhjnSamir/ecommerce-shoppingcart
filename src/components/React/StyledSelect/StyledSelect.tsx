...
interface Props {
  ...
  isDisabled?: boolean,
  options: OptionType[],
  onChange: (name: string, event: ValueType<OptionType, false>) => any,
...
function StyledSelect(props: Props) {
  ...
  return (
    <Select
      ...
      placeholder={placeholder || onInputChange ? getTextByLanguage("Type to Search...", "खोजी गर्न टाइप गर्नुहोस् ...") : getTextByLanguage("Choose Option...", "विकल्प छान्नुहोस ...")}
      id={id}
      name={name}
      onChange={(event: ValueType<OptionType, false>) => event ? onChange(name, event) : onChange(name, { label: "", value: "" })}
